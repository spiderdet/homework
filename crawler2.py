...
#此程序:爬虫2支持断点续传功能, 即当爬虫1 或爬虫2 爬行中断,保存了进度的时候, 可用爬虫2
#在之前的进度基础上继续爬取
...
def crawl2(tocrawl, crawled, max_page, ifpass):
    count = 0
...
def load():  #读取之前保存的进度
    file1 = open('tocrawl.txt')
    tocrawl = file1.read().rstrip().split('\n')
    file1.close()

    file2 = open('crawled.txt')
    crawled = file2.read().rstrip().split('\n')
    file2.close()
    return tocrawl, crawled
...
if __name__ == '__main__':
    ...
    ifpass = 1
    tocrawl , crawled = load()  #从之前进度读取并接着爬取数据

    start = datetime.now()
    crawl2(tocrawl, crawled, max_page, ifpass)
    end = datetime.now()
    print (end - start)